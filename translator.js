document.addEventListener('DOMContentLoaded', () => {
  ...
  // Queue for iterative translation
  ...
  // Process next iterative translation
  function processNextIterativeTranslation() {
    ...
    // Get oldest translation request
    const nextTranslation = pendingTranslations.shift();
    
    // Skip old interim translations
    if (!nextTranslation.isFinal && Date.now() - nextTranslation.timestamp > 1000) {
      // This is an old interim translation, we can skip it
      processNextIterativeTranslation();
      return;
    }
    
    // Process this translation
    translateText(nextTranslation.text, nextTranslation.isFinal)
      .then(() => {
        // Process next after a small delay to avoid overwhelming the API
        setTimeout(processNextIterativeTranslation, 300);
      })
      .catch(error => {
        console.error('Error in iterative translation:', error);
        // Continue with next translation even in case of error
        setTimeout(processNextIterativeTranslation, 500);
      });
  }
